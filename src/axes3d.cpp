...
// A limit pair that every downstream conversion can divide by. Rejected once
// here rather than guarded at each of the places that map data onto the box,
// which is the same bargain Axes::heatmap() struck for its extent.
...
// The caller's ErrorBar3D, length-checked and copied. The 2D
// take_error_bars()'s message, so a caller learns the same thing in the same
// words from either dimension.
ErrorBar3DData take_error_bars3d(const ErrorBar3D& eb, std::size_t n, const char* who) {
    ...
    ErrorBar3DData d;
    d.cap_lo[0] = take(eb.x_cap_lo, "x_cap_lo");  d.cap_hi[0] = take(eb.x_cap_hi, "x_cap_hi");
    d.box_lo[0] = take(eb.x_box_lo, "x_box_lo");  d.box_hi[0] = take(eb.x_box_hi, "x_box_hi");
    d.cap_lo[1] = take(eb.y_cap_lo, "y_cap_lo");  d.cap_hi[1] = take(eb.y_cap_hi, "y_cap_hi");
    d.box_lo[1] = take(eb.y_box_lo, "y_box_lo");  d.box_hi[1] = take(eb.y_box_hi, "y_box_hi");
    d.cap_lo[2] = take(eb.z_cap_lo, "z_cap_lo");  d.cap_hi[2] = take(eb.z_cap_hi, "z_cap_hi");
    d.box_lo[2] = take(eb.z_box_lo, "z_box_lo");  d.box_hi[2] = take(eb.z_box_hi, "z_box_hi");
    return d;
}

// The spacing a grid coordinate vector implies, as a bar footprint. Median
// would be more robust on an irregular grid, but the smallest gap is what
// stops bars overlapping, and a bar chart whose bars overlap reads as a bug
// rather than as irregular data. A single-column grid has no spacing to take,
// so it falls back to 1 -- the same answer Axes::bar() gives for one bar.
...
Axes3D& Axes3D::bar3d(PlaneOrientation orient,
    ...
    // Resolved once, here, so nothing downstream has to know a fraction was
    // involved -- the same bargain BarPlot::bar_width strikes.
...
Axes3D& Axes3D::surface(PlaneOrientation orient,
    ...
    // The one condition a bar grid does not have. A bar is drawn at a sample;
    // a surface is drawn *between* samples, so a 1 x n grid has vertices and
    // no cells -- and would render as nothing at all, which is the kind of
    // silence a caller reads as a bug in the library rather than in the call.
...
// The validation every surface_tri() overload runs on its vertices, before
// either the given topology or the derived one is looked at. One copy, because
// four overloads that check the same four things in four places is four
...
void check_mesh_vertices(std::span<const double> x, std::span<const double> y,
    ...
    // Three, not one: a mesh is made of triangles, and two vertices span no
    // area at all -- the same argument surface() makes for its 2 x 2 grid,
    // which is the smallest thing with a cell in it.
    if (x.size() < 3)
        throw std::invalid_argument(
...
// Builds the plot object once the topology is settled, whichever of the two
// ways produced it. `tri` is already validated against `x.size()`.
...
Axes3D& Axes3D::surface_tri(std::span<const double> x, std::span<const double> y,
    ...
    // Empty is rejected rather than accepted as "a mesh with no faces": a mesh
    // that draws nothing while still feeding the limits and the legend is the
    // silence line3d() rejects a one-point path for.
    if (tri.empty())
    ...
    // Degenerate (zero-area) triangles are deliberately *kept*. Dropping them
    // would make the caller's face count and ours disagree, and a face count
    // is how a caller checks their own mesh; the normal is guarded where it is
    // taken instead (see surface_tri_face()).
...
Axes3D& Axes3D::surface_tri(std::span<const double> x, std::span<const double> y,
    ...
    // The two axes `orient` names, through the same Axis3Map a bar grid and a
    // grid surface read their u and v out of. Unlike theirs, this is the last
    // thing that consults the orientation at all: it says how the mesh is
    // *made*, and what is stored afterwards is indices, so nothing about the
    // drawing ever asks which plane it was triangulated on.
    ...
    // **A concave domain gets spanned**, and that is a boundary rather than a
    // bug: Delaunay fills the convex hull, so a C-shaped point set comes back
    // with the gap bridged. matplotlib answers that with Triangulation.mask;
    // masking is deferred, and the header says so where a caller will read it.
...
Axes3D& Axes3D::line3d(std::span<const double> x, std::span<const double> y,
    ...
    // A path of one point is rejected where a cloud of one is not, and the
    // difference is not pedantry: a cloud of one point is a picture of that
    // point, while a path of one has no segment and so draws nothing at all.
    // Silently contributing to the limits and to the legend while putting no
    // ink in the box is the failure mode a throw at ingest exists to prevent.
    if (x.size() < 2)
...
Axes3D& Axes3D::set_ztitle(std::string_view text, float fontsize) {
    d->ztitle = text; d->axes_style.ztitle_fontsize = fontsize; return *this;
}
...
Axes3D& Axes3D::set_zlim(double lo, double hi) {
    ...
    d->zmin = lo; d->zmax = hi; d->zlim_auto = false; return *this;
}
...
Axes3D& Axes3D::set_box_aspect(BoxAspect a) {
    ...
    d->aspect = a; return *this;
}

// Every camera the caller can set goes through clamp_camera(), the same
// function the navigation and the panel end in -- elevation held off the pole
// where the basis collapses, zoom and field of view inside the range a slipped
// gesture cannot escape. One copy rather than a public-API variant of it, or
// the two would answer differently about the same camera the first time either
// gained a field.
...
Axes3D& Axes3D::set_camera(Camera3D cam) { d->camera = clamp_camera(cam); return *this; }
Camera3D Axes3D::camera() const { return d->camera; }

Axes3D& Axes3D::set_default_camera(Camera3D cam) {
    d->default_camera = clamp_camera(cam); return *this;
}

Axes3D& Axes3D::set_projection(Projection mode) {
    d->camera.projection = mode; return *this;
}
