// sextant -- painter split probe (v1.0 deferred plan)
//
// Built to answer one question: the gallery's sixth cell -- a sheet threaded
// through a grid of translucent bars -- reached `SvgExportOptions::max_splits`
// and raising the bound did not finish it. Was that scene merely *large*, or
// was the split loop failing to make progress on it?
//
// It was the second, and nothing that existed could tell the two apart.
// `SvgSaveReport` gives totals, and 44,850 cuts spread over 266 polygons is a
// hard scene while 44,850 cuts of *one* polygon is a bug. So this calls
// `paint_order()` directly -- no window, no GL context, no SVG writer -- and
// reads the ordered polygons back, grouped by the `source` they were cut from.
// `plan_scene3d()` reduces each of them to an index and a pixel ring, so that
// provenance never reaches a caller.
//
// It asserts nothing and is not a test: it prints a table. Kept because the
// same question will be asked again of the next scene that will not converge,
// and because the two tolerances the painter now depends on -- the minimum
// piece area and the projection-overlap slack -- were both chosen against
// numbers this prints.
//
...
// The gallery cell's own pixel size: 1500x950 over a 2x3 grid. The projection
// is what decides which polygons overlap on screen, so a probe at a different
// size would be a probe of a different scene.
constexpr int W = 500, H = 475;

// Every surface drawn with its wireframe on -- the third argument, "wire". The
// scene the first deferred-plan entry was reported against: the wire outline is
// an object of its own in the painter's soup, and the question is what it does
// to the order.
bool g_wire = false;
...
// `test_translucent3d`'s sixth cell, at grid size N. Everything except N is
// copied from tests/perf_test/main.cpp's gallery row.
...
FigureSnapshot snapshot_for(const Scene& sc) {
    FigureSnapshot fs;
    RenderSnapshot3D rs;
    // Limits left auto, which is what the gallery does. They decide the box
    // and so the projection, and a box sized by hand would be a different
    // scene wearing this one's name -- the sheet sits *inside* the bars here
    // (`midway` is half the bar's own height above its base), and how much of
    // the box a bar covers on screen is what decides which pairs overlap.
    rs.bars3d   = sc.bars;
...
// plan_scene3d()'s own soup, minus the planes this scene does not have. Built
// here rather than reached through plan_scene3d() so the *ordered polygons*
// survive: a ScenePaint keeps an index and a pixel ring and throws the rest
// away, and the rest is what this is measuring.
std::vector<PaintPoly> soup_for(const Projector3D& proj, const Scene& sc) {
    ...
    // Two points is a stroke -- a translucent bar's edge, a wireframe edge --
    // and plan_scene3d() orders those too, so the probe has to.
    for (std::size_t i = 0; i < bp.size(); ++i) {
        if (bp[i].box.size() < 2) continue;
        PaintPoly p;
        p.ring = bp[i].box; p.kind = PaintPoly::Kind::Bar;
        p.object = bp[i].plot; p.element = bp[i].bar; p.rank = i; p.source = i;
        soup.push_back(std::move(p));
    }
    for (std::size_t i = 0; i < sp.size(); ++i) {
        if (sp[i].box.size() < 2) continue;
        PaintPoly p;
        p.ring = sp[i].box; p.kind = PaintPoly::Kind::Surface;
        p.object = sp[i].plot; p.element = sp[i].cell; p.rank = i; p.source = i;
        soup.push_back(std::move(p));
    }
    return soup;
}
...
// What the pieces of the worst-cut polygons actually look like. A scene that
// is merely large produces pieces of comparable size; a loop that is shaving
// slivers off one polygon produces a long tail of near-zero areas, and the
// difference is visible in nothing the public path reports.
void detail(int N, std::size_t budget) {
    ...
    for (std::size_t i = 0; i < rank.size() && i < 5; ++i) {
        ...
        double sum = 0.0;
        std::size_t tiny = 0;
        for (double v : a) { sum += v; if (v < 1e-9) ++tiny; }
        char name[32];
...
// Newell's five tests, re-implemented here because painter3d.cpp's own copies
// live in an anonymous namespace.
//
// This is not duplication for its own sake: it answers the one question the
// stats cannot. When a pair comes out in the wrong order there are exactly two
// possible reasons -- a test said "separable" and was wrong, or the pair was
// never compared at all because the bucket grid or the depth-suffix stop
// filtered it out. Running the ladder on the offending pair by hand says
// which, and they call for opposite fixes.
struct Ladder { bool depth, bbox, behind, in_front, overlap; };
...
Ladder ladder(const PaintPoly& P, const PaintPoly& Q, const Projector3D& proj) {
    ...
    if (oriented(Q.ring, qp0, qn)) {
        ...
        for (const Vec3& v : P.ring)
            if (dot(v - qp0, qn) > 1e-7) { L.behind = false; break; }
    }
    if (oriented(P.ring, pp0, pn)) {
        L.in_front = true;
        for (const Vec3& v : Q.ring)
            if (dot(v - pp0, pn) < -1e-7) { L.in_front = false; break; }
    }
    // Separating-axis on the projections, with painter3d.cpp's own slack.
...
// The ray-cast oracle `test_scene3d_svg_order()` judges the emission order
// with, run here so that a pixel it calls out of order can be *described*: the
// test reports how many there are, and what is wanted while diagnosing is
// which polygons they are and what shape those polygons are in.
//
// Same method: walk the emitted order at one pixel, intersect the pixel's ray
// with each emitted polygon's own plane, and check the depths come out
// non-increasing. The plane is the parent's, which a piece is coplanar with by
// construction.
void wrong_pixels(int N, int w, int h) {
    ...
    for (float y = fr.y; y < fr.y + fr.h; y += 3.0f)
        for (float x = fr.x; x < fr.x + fr.w; x += 3.0f) {
            struct Hit { const PaintPoly* p; float d; };
            ...
            for (std::size_t i = 0; i < stack.size(); ++i) {
                ...
                std::printf("    %c%-4zu el%-5zu %s depth %9.5f  area %10.3e  ring %zu%s\n",
                            ...
                            (i && stack[i].d > stack[i - 1].d + 3e-3f) ? "   <-- OUT OF ORDER"
                                                                      : "");
                if (!i || stack[i].d <= stack[i - 1].d + 3e-3f) continue;
                // P is the one emitted first, so P is what the algorithm
                // believed was farther. Any of these five being true is the
                // algorithm's reason for believing it.
...
int main(int argc, char** argv) {
    ...
    for (int N = 3; N <= maxN; ++N) {
        ...
        std::printf("%3d %7zu %9zu %9zu %12zu %9.0f %7zu %6zu%c %7zu %s\n",
                    ...
                    r.st.bailed ? (r.st.bailed_on_splits ? "BAILED(splits)"
                                                         : "BAILED(work)") : "");
        std::fflush(stdout);
        if (r.st.bailed) { detail(N, 3000); std::fflush(stdout); break; }
    }

    // And, at the size and camera `test_scene3d_svg_order()` judges it at,
    // which pixels the order gets wrong.
    wrong_pixels(13, 460, 420);

    // A smooth sheet on its own: a single-valued height field seen from above
    // occludes itself nowhere, so **this must never split**. It is the control
    // for how tight the projection-overlap slack may be -- adjacent cells of
    // one grid share their two corner samples exactly, so a slack of any size
    // separates them, but shrink it far enough and cells that merely *pass
    // near* each other on screen start conflicting and the sheet acquires cuts
    // that answer nothing.
    {
        constexpr int M = 50;
        Scene sc;
    ...
    // The two controls. Neither object interleaves with itself, so a split in
    // either of these is a split that has nothing to do with the other one.
