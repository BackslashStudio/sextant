// Plane2D: ingest, the reference raster, the quad, and decoration hoisting.
//
...
// ===========================================================================
// v1.0 step 5 -- Plane2D: the 2D kinds in a 3D scene
// ===========================================================================
...
void test_plane2d_ingest() {
    ...
    // Placement is placement: cla() drops what is on the plane, not where it is.
    ...
    // What is rejected, and where. Every message names Plane2D, so a caller
    // who mixed up an axes and a plane is told which one complained.
    ...
    check(threw_with([&]{ ax->plane(PlaneOrientation::XY,
    ...
    // Contours were refused at ingest in step 5, on the grounds that a heatmap
    // silently coming out without the lines it asked for reads as a broken
    // colormap. Step 6 draws them, so the refusal is gone and the ingest takes
    // them on exactly the 2D terms -- sorted, de-duplicated, non-finite
    // rejected.
    {
        HeatmapOptions ho;
        ho.contours = { 0.75, 0.25, 0.75 };
        pl->heatmap(d, 3, 4, { 0, 1 }, { 0, 1 }, ho);
        check(threw_with([&]{ HeatmapOptions bad;
    ...
    // imshow is the index-space specialization here too, not a second ingest
    // path -- so a plane drawn by each must land on exactly the same pixels.
    // Two planes at one offset, one of each, and the check is that the file
    ...
    // Planes and bar3d coexist in one axes -- the box is a scene, not a slot --
    // and all of it has to survive the trip through the snapshot.
...
void test_plane_raster() {
    ...
    Transform3D tf;
    tf.xmin = -4.0; tf.xmax = 4.0;     // span 8
    tf.ymin =  0.0; tf.ymax = 2.0;     // span 2
    tf.zmin = -1.0; tf.zmax = 3.0;     // span 4
    tf.aspect = { 1.0, 0.5, 0.75 };

    // ---- Shape comes from the box, not from the data and not from the frame.
    // The two spans above are deliberately unlike the two aspect sides, so a
    // raster sized from the data would be visibly the wrong shape here.
    {
    ...
    // A cube gives a square raster whatever the data spans are -- the negative
    // control for "shape comes from the box", since data-sized would not.
    {
        Transform3D cube = tf;
    ...
    // Never below one texel, however flat the box is squashed.
    {
        Transform3D flat = tf;
    ...
    // ---- The transform is the parent's limits on the two spanned axes, onto
    // the raster. Not the data's bounds: a plane's u and v *are* two of the
    // parent's axes, so anything outside them is off the box and clips.
    {
        ...
        // The two corners the frame maps to, computed here rather than read
        // back from the same expression the transform is built out of.
    ...
    // ---- The quad spans the box face at the offset, and its texture is the
    // right way up. This is the pairing that inverts silently: the raster's
    // v_min is at the bottom, and a rendered framebuffer's t = 0 is its bottom
    // row, so v_min must be t = 0 -- the opposite of plane_heatmap_quad(),
    // whose texture is an uploaded image with row 0 at the top.
    {
        const PlaneRaster r = plane_raster(tf, PlaneOrientation::XY, 1.5, 800);
        ...
        // The pairing itself, asserted rather than the two halves separately:
        // for every corner, the pixel the transform sends that corner's data
        // to must be the pixel the texture coordinate names. That is what
        // catches an inverted t, which the two checks above would survive if
        // both were inverted together.
        for (int i = 0; i < 4; ++i) {
            const double u = (i == 1 || i == 2) ? tf.xmax : tf.xmin;
            const double v = (i >= 2)           ? tf.ymax : tf.ymin;
            const float sx = r.tr.to_px(u) / r.tr.pw;
            // Pixel y down, texture t up, hence the flip on this one only.
    ...
    // ---- A reversed limit mirrors the raster, and the quad follows it,
    // because both are keyed on the declared limits rather than on min/max.
    {
        Transform3D rev = tf;
        rev.xmax = -4.0; rev.xmin = 4.0;
        ...
        // Same pairing check, which is the one that would fail if only one of
        // the two had been reversed.
        for (int i = 0; i < 4; ++i) {
    ...
    // ---- The camera is not an input. That is the property that keeps an
    // orbit from invalidating the plane's texture, so it is asserted rather
    // than left to the signature: the raster depends on the transform, the
    // orientation, the offset and the cap, and on nothing else.
    {
...
void test_plane2d_geometry() {
    ...
    // ---- The quad. Plane coordinates *are* parent coordinates, so a corner
    // is the extent's own value on the two spanned axes and the offset on the
    // third -- no stretching to a box face, which is the alternative §6
    // rejected.
    HeatmapPlot hp;
    hp.rows = 2; hp.cols = 2;
    ...
    // The texture assignment is copied from the 2D quad rather than re-decided,
    // which is what makes "the same heatmap, drawn two ways" a statement.
    ...
    // ---- The pixels. Row 0 of the buffer is the yrange.hi edge whichever way
    // `origin` puts the data, which is the one rule the texture upload and the
    // <image> encode both depend on.
    ...
    HeatmapPlot hl = hp; hl.opts.origin = "lower";
    HeatmapPlot hu = hp; hu.opts.origin = "upper";
    ...
    // ---- Auto-scale. This is the consequence of the coordinate decision: a
    // plane's data feeds the parent's limits directly, along the two axes it
    // spans, with the offset landing on the third.
    ...
    // A plane is flat, so the third axis gets a single value and falls into
    // auto_scale3d's own degenerate-interval rule -- centred on the offset
    // rather than collapsed onto it, which is what keeps the box a box.
    ...
    // An empty plane has nothing to scale to -- and must not drag the third
    // axis to its offset either. "Auto" means "from the data".
    ...
    // The union with a bar3d object, which is the case the shared axis array
    // exists for.
    Bar3DPlot bar;
...
void test_plane2d_decoration_hoist() {
    ...
    // Taller than it is wide on purpose: the fit takes the *smaller* of the
    // two scales, so only in a frame the box is already width-limited in does
    // narrowing it have to change the picture. In a wide frame the height
    // binds and a correct carve legitimately leaves the box the same size,
    // which would make the check below pass for the wrong reason.
    constexpr int W = 320, H = 420;

    HeatmapOptions cb;
    cb.colorbar = true;
    cb.vmin = 0.0f; cb.vmax = 100.0f;
    ...
    // The box is fitted to what is left, which is the whole point of carving
    // before building the projector rather than after.
    auto box_width = [](const FigureLayout& l) {
        const Projector3D& p = l.cells[0].box3d->proj;
        float x0 = 1e9f, x1 = -1e9f;
        for (int i = 0; i < 8; ++i) {
            const Px3 q = p.project_box({ (i & 1) ? 0.5 : -0.5, (i & 2) ? 0.5 : -0.5,
                                          (i & 4) ? 0.5 : -0.5 });
    ...
    // And the inverse subtracts what the forward direction added -- the check
    // that the two are one piece of arithmetic and not two.
    ...
    // Both outputs draw it, out of the scene and beside the frame.
...
// A 3D colorbar's styling is the axes' own (v1.0 step 11.2). It used to be
// read off whichever plane had asked for the bar -- which could only ever be
// the default, since a Plane2D has no set_colorbar_style() to reach the
// ColorbarOptions it holds, so a 3D colorbar was unstylable however it was
// approached. The checks below are therefore about a capability that did not
// exist rather than about one that changed: on the old code the styling is
// pinned at its default and every width here comes out the same.
void test_axes3d_colorbar_style() {
    ...
    // The block is the strip plus room for the bar's own numbers, so a bigger
    // font is a wider block -- which is the observable that says the styling
    // reached the measurement and not just the drawing.
    ...
    // A plane's own colorbar_opts is now inert -- which is the point: it was
    // the only thing the layout read before, and nothing could set it.
    ...
    // Through the public API, all the way into a file -- the only way a
    // caller has, and the one that was missing entirely.
