// Axis position in 2D (v1.0 step 19): AxesStyle's placement enums and origin
// components, what they cost the insets, and what each output draws from them.
//
...
// One 2D axes over fixed 0..10 limits carrying a two-point line, so every
// expectation can be stated through the cell's own transform. Limits are
// explicit by default; the auto-limit checks turn that off themselves.
FigureSnapshot one_axes(AxesStyle st = {}) {
    FigureSnapshot fs;
    FigureAxesSnapshot fa;
    fa.slot = AxesSlot{ 1, 1, 1 };
    RenderSnapshot& s = *fa.snap2d();
    s.xmin = 0.0; s.xmax = 10.0; s.xlim_auto = false;
    s.ymin = 0.0; s.ymax = 10.0; s.ylim_auto = false;
...
// -------------------------------------------------------------------------
// place_axis(), the one piece of policy both dimensions share
// -------------------------------------------------------------------------
void test_axis_placement() {
    ...
    // Mid reads the resolved range, so an asymmetric one still lands between.
    ...
    // ...and clamps rather than leaving the frame, in both directions.
    ...
    // A range with nowhere to be in between: everything collapses onto Low,
    // which is the only placement that still means something.
    ...
    // widen_for_origin only touches bounds that are actually automatic.
...
void test_axis_position_layout() {
    ...
    // Mid lands on the transform's own midpoint, and becomes its own line.
    AxesStyle mid_st; mid_st.xaxis_y = mid_st.yaxis_x = AxisPosition::Mid;
    ...
    // An interior axis draws its band inside the frame, so it reserves none
    // of the cell -- and the frame it gives back is what the band cost.
    ...
    // High moves the band to the far side rather than giving it back, and
    // flips the ticks so they still point away from the data.
    AxesStyle hi_st; hi_st.xaxis_y = hi_st.yaxis_x = AxisPosition::High;
    ...
    // Not its width, though, and that is correct rather than a rounding
    // slip: the left and right insets also hold half of the first and last x
    // label, and "0" is narrower than "10". Swapping the y band between the
    // two sides therefore swaps which of the two halves it swallowed. What
    // matters is that the band was moved and not handed back.
    check(hi.frame.w < mid.frame.w, "High: and gives up a band's worth of width, unlike Mid");

    // The label anchors follow the ticks: below the line at Low, above it at
    // High, by the same offset in each case.
    ...
    // Titles do not move with the axis: both stay in the cell's outer band.
...
void test_axis_origin_and_limits() {
    ...
    // Automatic limits over data well away from zero: pinning to zero has to
    // widen the view, or the line it asks for could not be drawn at all.
    auto fs = one_axes({});
    ...
    // The padding is the one auto_scale would have applied, so a pinned
    // figure's limits are what an unpinned one over the same widened data
    // would be -- no second rule for how much room a pin gets.
    ...
    // Explicit limits win, and the axis clamps onto the edge rather than
    // disappearing off it.
    s.xlim_auto = s.ylim_auto = false;
    s.xmin = s.ymin = 5.0; s.xmax = s.ymax = 10.0;
    ...
    // One component alone is meaningful: it moves the axis it is measured
    // across and leaves the other where its enum put it.
    AxesStyle one;
    one.origin_y = 5.0;
...
void test_axis_position_rendered() {
    ...
    // No box, so the only dark ink left in the frame is the two axis lines
    // and the data -- which this case keeps well away from the middle.
    ...
    // Dark ink, i.e. a spine or an axis line (0.3 grey) rather than the white
    // ground or the blue data line.
    auto load = [&](const char* name, const std::function<void(std::function<bool(float, float)>)>& body) {
        ...
        if (!px) return;
        // A 1 px stroke centred on a half-pixel covers one row, on a whole
        // pixel it straddles two at half weight -- and the axis lines land on
        // whatever the transform says, not on a grid. So the probe looks in a
        // one-pixel neighbourhood rather than at a single sample. That is
        // still far tighter than any distance these checks care about: the
        // frame is hundreds of pixels across.
        body([&](float x, float y) {
            ...
            for (int dy = -1; dy <= 1; ++dy) for (int dx = -1; dx <= 1; ++dx) {
                ...
                // 200, not the spine's own 77: a 1 px line rarely lands on a
                // pixel centre, so its ink is split between two rows and each
                // is a half-coverage blend with white. The colour test is
                // what keeps this from matching the data line -- grey has its
                // channels together, blue does not.
    ...
    load("ax_mid", [&](auto dark) {
        // The two lines are where the layout said, a quarter and three
        // quarters along the frame -- away from every edge.
        ...
        // With every spine off, the frame's own edges carry no ink at all.
        ...
        // The ticks hang below the interior line, inside the frame. 3 px
        // down is clear of the line's own neighbourhood and still inside the
        // 5 px mark; 3.0 is a tick, the limits being 0..10 at step 1.
    ...
    // The SVG has to agree, or the two paths have drifted. It writes the
    // interior axes as one <path>, so the coordinates are readable directly.
    ...
    // Every spine off means no outline path at all -- not an empty one.
    const std::size_t mid_paths = [&] {
        std::size_t n = 0, at = 0;
        while ((at = mid_svg.find("<path d=\"M", at)) != std::string::npos) { ++n; at += 4; }
        return n;
    }();
...
// -------------------------------------------------------------------------
// The 3D half (v1.0 step 20): two coordinates per axis, and Auto meaning the
// camera rather than Low.
// -------------------------------------------------------------------------
void test_axis_placement3d() {
    ...
    // A pin beats Auto, which is the only way Auto ever stops being the
    // camera's. Both dimensions agree on that ordering.
    ...
    // Clamping is per coordinate, so two out-of-range components land the axis
    // on a box edge and one lands it on a face -- never on "the nearest of
    // twelve edges", a question this design never has to answer.
...
void test_axis_position_3d_plan() {
    ...
    auto ends = [](const Box3DPlan::Poly& p) {
        return std::pair{ std::pair{ p.xy[0], p.xy[1] },
    ...
    // Every axis pinned to the middle of its two other coordinates: with
    // 0..1 limits that is the box centre, so all three lines cross there.
    AxesStyle cross;
    ...
    // The lines moved; the titles did not. A 3D cell reserves nothing outside
    // its frame, so a title has no outer band to fall back to -- it stays on
    // the camera's silhouette edge, which is what step 20 settled.
    ...
    // And so does the direction the labels are pushed, borrowed from that same
    // edge -- which is the point of borrowing it. Taken from the line itself,
    // an axis through the centre has a zero-length vector and no direction
    // at all.
    bool same_push = true;
    ...
    // A single component is meaningful on its own: origin_z drops the x and y
    // axes to z = 0 and leaves the z axis where the camera had it. That is
    // what nine shared fields buy over an all-or-nothing Point3.
    AxesStyle zonly;
    zonly.origin_z = 0.5;
    ...
    // box_y(0.0) is -h.y, the Auto edge's y under this camera: the component
    // that was not set is still the camera's.
    ...
    // Low and High are absolute, which is the whole reason Auto has to be a
    // distinct enumerator: turn the camera and an Auto edge moves, a High one
    // does not.
    Camera3D turned = snap.camera;
    turned.azimuth += 180.0;
    // The x axis' two endpoints in box space, which is where "did it move"
    // can be asked without a camera in the way.
    auto x_ends_box = [&](const Camera3D& cam, const AxesStyle& st) {
        ...
        const Box3DPlan p = plan_box3d(pr, s, *snap.xticks_override, *snap.yticks_override,
                                       *snap.zticks_override);
        // Back out of pixels: the line is axis-aligned in box space, so its
        // own endpoints are what the two other coordinates were set to.
    ...
    // Turn the camera half a circle and ask the same question twice. A High
    // edge is the same box edge; an Auto one is not -- which is exactly why
    // Auto cannot be a synonym for Low here the way it is in 2D.
...
void test_axis_origin_3d_limits() {
    ...
    // One point well away from the origin on all three axes, everything
    // automatic: pinning to zero has to pull the box out to reach it.
    ...
    // The unpinned path must be arithmetically what it was before pins
    // existed, or every 3D picture in the suite moves a little.
    ...
    // An explicit limit is never widened: the pin clamps into it instead.
    AxesStyle zpin2 = zpin;
    FigureSnapshot fs = figure(zpin2);
    RenderSnapshot3D& s = *fs.axes[0].snap3d();
    s.zlim_auto = false; s.zmin = 5.0; s.zmax = 10.0;
