// The vector scene: Newell's algorithm, the splits, and the export budget.
//
...
// -------------------------------------------------------------------------
// Newell's algorithm: an order where no sort has one (v1.0 step 9)
// -------------------------------------------------------------------------
// Three scenes, and the same oracle for all of them: a **ray cast**.
//
// For each of a grid of pixels the test asks `Projector3D::ray_from_pixel()`
// which emitted polygons that pixel's ray actually meets and at what depth,
// then requires the emission order restricted to those to run strictly back to
// front. That is derived from the geometry and from nothing the algorithm
// computed -- no comparator, no depth key, no centroid -- which is §13's rule
// and the only kind of check worth making about an ordering.
//
// The three scenes are chosen for what each proves:
//
//   1. Two quads crossing. No sequence of two whole polygons is right, so a
//      correct result *must* contain a split. This is the smallest scene the
//      whole step exists for.
//   2. A pinwheel: three long quads, none intersecting, each in front of the
//      next and behind the one after. The relation is not a total order at
//      all, so it cannot merely be sorted wrongly -- there is nothing to sort
//      into. Splitting is what gives it an answer.
//   3. Two quads clear of each other. Nothing may be split, and the order must
//      be the depth order. Without this, a painter that split everything in
//      sight would pass the other two.
// -------------------------------------------------------------------------
// The gallery's own interleaving cells, ordered for the SVG (v1.0 step 9)
// -------------------------------------------------------------------------
// `test_translucent3d`'s three hardest cells, checked against the ray cast
// rather than against a picture. They are here because comparing the SVG with
// the window by eye is what found the defect these assert against, and an eye
// is not a check that runs again.
//
// The defect was one thing wearing two faces. **A surface cell's four samples
// are not coplanar** -- on this ripple they miss a common plane by 10-18% of
// the cell's own width -- and every one of Newell's tests asks which side of a
// polygon's plane something lies on. A quad with no plane answers with an
// average that contains none of its own vertices, so pairs never resolved and
// the painter split them over and over: 145 polygons produced 1,225 splits and
// ran into its work bound. Emitting the two triangles the raster path already
// draws fixes the ordering *and* removes a divergence nobody had noticed --
// for a warped cell the two outputs had been drawing different shapes.
//
// The second face was the bound itself. The bar-grid cell needs ~894,000 pairwise
// tests and was quietly stopping at 400,000, which is a silently wrong picture.
// It is now bounded far above what these scenes need, and a bail writes a
...
void test_scene3d_svg_order() {
    ...
    auto base = [&] {
        ...
        s->camera.azimuth = -55.0; s->camera.elevation = 24.0;
    ...
    auto add_surface = [&](RenderSnapshot3D* s, const std::vector<double>& h) {
        ...
        sp.opts.colormap = true; sp.opts.alpha = 0.6f;
        s->surfaces.push_back(std::move(sp));
    };
    auto add_plane = [&](RenderSnapshot3D* s, double off) {
        PlaneSnapshot pl;
        pl.orient = PlaneOrientation::XY; pl.offset = off; pl.opts.alpha = 0.6f;
        HeatmapPlot hp; hp.rows = 24; hp.cols = 24;
        ...
        hp.xrange = { -3.0, 3.0 }; hp.yrange = { -3.0, 3.0 };
        pl.sheet.heatmaps.push_back(std::move(hp));
        s->planes.push_back(std::move(pl));
    };

    // What the writer will emit, as (pixel ring, screen bbox, box-space plane).
    // The plane comes from the *parent* polygon, which a piece is coplanar with
    // by construction, so a split changes the outline and not the plane.
    struct Emitted { std::vector<float> xy; float bb[4]; Vec3 p0, n; bool ok = false; };
    ...
    auto run = [&](const char* what, FigureSnapshot& fs) {
        ...
        for (const ScenePaint& e : scene) {
            ...
            if (e.kind == ScenePaint::Kind::Bar && e.index < bp.size()) {
                ...
                for (const PlanePlanItem& it : pp)
                    if (it.plane == e.index) { quad.assign(it.quad, it.quad + 4); break; }
                parent = &quad;
                if (!e.xy.empty()) x.xy = e.xy;
                else {
                    std::vector<Px3> r;
                    pj.project_polygon(quad, r);
                    for (const Px3& q : r) { x.xy.push_back(q.x); x.xy.push_back(q.y); }
                }
            }
            x.bb[0] = x.bb[1] = 1e30f; x.bb[2] = x.bb[3] = -1e30f;
            for (std::size_t i = 0; i + 1 < x.xy.size(); i += 2) {
                x.bb[0] = std::min(x.bb[0], x.xy[i]);     x.bb[1] = std::min(x.bb[1], x.xy[i + 1]);
                x.bb[2] = std::max(x.bb[2], x.xy[i]);     x.bb[3] = std::max(x.bb[3], x.xy[i + 1]);
            }
        ...
        for (float y = fr.y; y < fr.y + fr.h; y += 3.0f)
            for (float x = fr.x; x < fr.x + fr.w; x += 3.0f) {
                ...
                for (const Emitted& e : em) {
                    ...
                    ++hits;
                    if (have && d > prev) { worst_gap = std::max(worst_gap, (double)(d - prev));
        ...
        if (newell_on) {
            ...
            // Zero, not "nearly all". These three were at 28, 32 and 0 while
            // the early termination was still unsound, and any threshold
            // loose enough to pass that would have called it fixed.
            check(wrong == 0,
    ...
    {   // The same cell at the **default** camera, which is a different scene
        ...
        //
        // The cut above is at (-55, 24) and this is at (-60, 30), and that six
        // degrees was the difference between an export that finished and one
        // that never could. A bar face already cut by one surface triangle has
        // an edge lying exactly on that triangle's plane -- and splitting the
        // triangle makes more polygons with the *same* plane, each of which
        // then "cut" the face into itself plus a ring of zero area. Whether
        // any such pair is ever compared is decided by whether they overlap on
        // screen, which is decided by the camera: at (-55, 24) none did, and
        // at (-60, 30) one did, and that one face reached 44,653 pieces.
        //
        // So the camera is part of this scene's definition. It is the view
        // `test_translucent3d` and the perf gallery both draw, which is where
        // this was found -- as an export that hit `max_splits` and could not
        // be bought out of it at twenty times the budget.
        FigureSnapshot fs = base();
        RenderSnapshot3D* s = fs.axes[0].snap3d();
    ...
    // The control for the *other* side of the projection-overlap tolerance.
    //
    // Everything above says the painter must not miss an overlap. This says it
    // must not invent one: a single-valued height field seen from anywhere
    // occludes itself nowhere, so a smooth sheet on its own has no pair to
    // resolve and must come out in exactly the order `surface_draw_order()`
    // put it in, uncut. Adjacent cells share their corner samples exactly, so
    // any positive slack separates *those* -- what this catches is a slack
    // small enough that cells merely passing near each other on screen start
    // conflicting, which is how a scene with nothing to answer acquires
    // geometry. It splits at a slack of 0.10 and does not at 0.15.
    {
        constexpr int M = 50;
        FigureSnapshot fs = base();
        RenderSnapshot3D* s = fs.axes[0].snap3d();
        ...
        SurfacePlot sp;
        sp.u = gu; sp.v = gv; sp.heights = CowVec<double>(std::move(h));
...
// -------------------------------------------------------------------------
// A surface's wireframe in the SVG (post-step-10.1 deferred item)
// -------------------------------------------------------------------------
// Two faults, and both went unseen because nothing exported a wireframe to SVG.
// The plan emitted each cell's outline as one closed four-point ring, and a
// cell's four corners have no common plane: the painter cut that ring by a
// plane three of its corners lay on, got the ring back *plus* a copy of one of
// its triangles, took that for progress, and split until its bound. And the
// writer drew every such item as a *filled* polygon of a default-constructed
// Color, so each cell, and each piece of one, came out black.
//
// The wireframe is now the cell edges, one two-point stroke each, which the
// painter orders against every polygon. `test_scene3d_svg_order()`'s oracle
// cannot see a stroke -- it has no plane to cast a ray at -- so this one asks
// the stroke's own question: wherever a polygon covers a point of a line,
// the polygon is drawn after the line exactly when it is nearer.
void test_scene3d_svg_wireframe() {
    ...
    auto base = [&] {
        FigureSnapshot fs = make_snapshot3d(1, 1, 1);
        RenderSnapshot3D* s = fs.axes[0].snap3d();
        // Orthographic, so a pixel's depth along a projected line is the
        // linear interpolation of its ends -- which is what lets the oracle
        // below take a point *on a piece* back to box space exactly.
        s->camera.projection = Projection::Orthographic;
        s->camera.azimuth = -60.0; s->camera.elevation = 30.0;
        s->box_style.panes = false; s->grid_enabled = false;
        s->xticks_override = std::vector<Tick>{};
    ...
    auto add_surface = [&](RenderSnapshot3D* s, const std::vector<double>& h, float alpha) {
        SurfacePlot sp;
        sp.u = g; sp.v = g; sp.heights = h;
        sp.opts.colormap = true; sp.opts.alpha = alpha;
        sp.opts.edges = true; sp.opts.edgecolor = kWire; sp.opts.edge_linewidth = 1.0f;
        s->surfaces.push_back(std::move(sp));
    };
    auto add_bars = [&](RenderSnapshot3D* s, float alpha) {
        Bar3DPlot b;
        b.u = g; b.v = g; b.heights = ripple;
        b.u_width = b.v_width = 0.35;
        b.opts.color = Color::Orange; b.opts.alpha = alpha; b.opts.bottom = -1.5;
        s->bars3d.push_back(std::move(b));
    };
    ...
    auto run = [&](const char* what, FigureSnapshot& fs) {
        ...
        // The plan: strokes are two points, and there is one per grid edge.
        ...
        struct Poly { std::size_t at; std::vector<float> xy; Vec3 p0, n; };
        struct Line { std::size_t at; float x0, y0, x1, y1; std::size_t src; };
        ...
        for (std::size_t at = 0; at < scene.size(); ++at) {
            ...
            if (e.kind == ScenePaint::Kind::Bar) {
                ...
                ring = &bp[e.index].box; filled = bp[e.index].filled;
            } else if (e.kind == ScenePaint::Kind::Surface) {
                xy = e.xy.empty() ? &sp[e.index].xy : &e.xy;
                ring = &sp[e.index].box; filled = sp[e.index].filled;
            } else continue;
            if (!filled) {
        ...
        // Strictly inside by a margin: a wireframe edge lies *on* the edge of
        // its own two triangles, and a point on a shared boundary has no
        // order to get wrong.
        ...
        for (const Line& l : lines) {
            ...
            for (const float t : { 0.2f, 0.5f, 0.8f }) {
                ...
                for (const Poly& p : polys) {
                    ...
                    ++covered;
                    // Larger depth is farther. A polygon drawn after the line
                    // must be nearer; one drawn before must be farther.
        ...
        // And through the writer: no black cells, and the wire is lines.
        SvgSaveReport rep;
    ...
    {   // The lone-sheet control, with the wire on: a height field that
        ...
        SurfacePlot sp;
        sp.u = gu; sp.v = gu; sp.heights = CowVec<double>(std::move(h));
        sp.opts.colormap = true; sp.opts.edges = true; sp.opts.edgecolor = kWire;
        s->surfaces.push_back(std::move(sp));
        const FigureLayout lay = compute_figure_layout(fs, W, H);
        const Projector3D& pj = lay.cells[0].box3d->proj;
        PaintOrderStats st;
...
// -------------------------------------------------------------------------
// The bounds as an option, and the admission when one binds (post-step-9)
// -------------------------------------------------------------------------
// The painter gives up on a scene it cannot finish inside its budget, and
// before this the only trace was an XML comment nobody reads. Three things are
// checked here, and they are the three that make the option worth having:
// the budget is *obeyed*, the report *says* it was hit, and the sentence names
// the bound that bound -- because a warning that reports a number without
// saying which knob takes it leaves the reader no better off.
//
// Deliberately through the public `Figure::savefig_svg()` rather than through
// `plan_scene3d()`, since what is being tested is the wiring from an option a
// caller sets to a report a caller reads. SVG export builds no GL context, so
// this stays a headless test like the rest of this file.
void test_export_budget() {
    ...
    // memory/spec_3d.md §10's own reproducer: a YZ plane cutting a bar grid
    // that spans it. Small, and known to need exactly a handful of cuts.
    auto make = [] {
        auto fig = Figure::create({ .width = 420, .height = 360,
                                    .title = "budget", .vsync = false });
        auto ax = fig->add_subplot3d(1, 1, 1);
        ax->set_view(-55.0, 24.0);
        // An *odd* grid, so a bar sits centred on x = 0 and the plane cuts
        // through it rather than passing between two rows. With an even one
        // nothing straddles the plane, nothing needs cutting, and the whole
        // scene comes out exact at zero splits -- which is a correct answer to
        // a question this test is not asking.
        ...
        ax->bar3d(PlaneOrientation::XY, u, v, z,
                  { .color = Color::Orange, .width = 0.7f, .depth = 0.7f,
                    .bottom = -1.0 });
        // The plane carries a heatmap because an *empty* plane emits no plan
        // item at all -- nothing to order, nothing to cut, and a budget test
        // whose scene never splits tests nothing.
    ...
    // Under the negative control there is no painter to bound: plan_scene3d()
    // returns the whole-object order and reports nothing. The budget is then
    // *inert*, and that is worth asserting rather than skipping -- a knob that
    // quietly did something in a mode with no algorithm behind it would be a
    // worse bug than one that did nothing in the mode that has one.
    bool newell_on = true;
    if (const char* env = std::getenv("SEXTANT_NEWELL"))
        newell_on = std::atoi(env) != 0;
    if (!newell_on) {
    ...
    // 1. The default finishes this scene, and says so.
    ...
    // 2. A budget below what it needs binds, and the report admits it. The
    //    file is still written -- that is the whole contract.
    ...
    // 3. The other bound is separately reachable, and separately named --
    //    without this, one budget could be silently doing both jobs.
    ...
    // 4. And raising it past the need is the same as the default, which is
    //    what makes the knob safe to reach for: it can only ever buy more
    //    work, never a different picture.
...
void test_painter3d() {
    ...
    // A quad from four data-space corners, tagged as its own object so the
    // same-object short circuit never hides a pair this test is about.
    ...
    // Where a pixel's ray meets a polygon's plane, as Px3::depth, or false if
    // the ray misses the polygon. Point-in-polygon is asked in *projection*
    // and the depth comes from the plane intersection, so neither half of the
    // answer comes from the painter.
    ...
    // The oracle. Walks a pixel grid over the union of the emitted polygons'
    // bounding boxes and requires that, wherever a ray meets more than one of
    // them, the later-emitted one is the nearer. `slack` forgives the pixel
    // or two along a split seam, where a ray legitimately grazes both pieces.
    auto check_order = [&](const std::vector<PaintPoly>& out, const char* what) {
        float x0 = 1e30f, y0 = 1e30f, x1 = -1e30f, y1 = -1e30f;
        for (const PaintPoly& p : out) {
            x0 = std::min(x0, p.bb[0]); y0 = std::min(y0, p.bb[1]);
            x1 = std::max(x1, p.bb[2]); y1 = std::max(y1, p.bb[3]);
        }
        ...
        for (float y = y0; y <= y1; y += 1.0f)
            for (float x = x0; x <= x1; x += 1.0f) {
                ...
                for (const PaintPoly& p : out) {
                    ...
                    ++hits;
                    // Emission is back to front, so depth must not increase.
    ...
    // ---- 1. Two crossing quads: no whole-polygon order exists
    {
        ...
        in.push_back(quad(0, { {0.1, 0.5, 0.15}, {0.9, 0.5, 0.15},
                               {0.9, 0.5, 0.85}, {0.1, 0.5, 0.85} }));   // upright, y = 0.5
        in.push_back(quad(1, { {0.5, 0.1, 0.15}, {0.5, 0.9, 0.15},
                               {0.5, 0.9, 0.85}, {0.5, 0.1, 0.85} }));   // upright, x = 0.5
        PaintOrderStats st;
        ...
        // The negative control, and the reason the oracle is worth its length:
        // the *unsplit* pair, in either whole-object order, must fail it. A
        // check that only ever sees the algorithm's own output cannot tell a
        // correct painter from a lenient oracle.
        int failed_both = 0;
    ...
    {
        // Three flat boards laid along the sides of a triangle, each rising
        // from `zlo` at the vertex it starts on to `zhi` at the vertex it
        // ends on. So where board k meets board k+1 -- at the vertex they
        // share -- k is at its high end and k+1 at its low one, and k is above
        // k+1. That holds for all three, round the cycle: A over B over C over
        // A. Nothing intersects anything, and there is still no order.
        std::vector<PaintPoly> in;
    ...
    // ---- 3. Two quads clear of each other: nothing may be split
    {
        std::vector<PaintPoly> in;
        // Close together in z, because two horizontal sheets far apart project
        // to two shapes that do not overlap on screen at all -- and a scene
        // with nothing overlapping proves nothing about an ordering.
        in.push_back(quad(0, { {0.1, 0.1, 0.45}, {0.9, 0.1, 0.45},
                               {0.9, 0.9, 0.45}, {0.1, 0.9, 0.45} }));
        in.push_back(quad(1, { {0.1, 0.1, 0.55}, {0.9, 0.1, 0.55},
    ...
    // ---- 4. §10's own reproducer, through the real planners
    //
    // A vertical plane cutting through a bar grid rather than clear of it --
    // the scene `memory/spec_3d.md` §10 names as the SVG's standing flaw, and
    // the one `test_plane2d.svg`'s third cell draws. The whole-object order
    // emitted the plane either before every bar face or after every one of
    // them, so half the grid came out on the wrong side of it.
    //
    // Checked by the same ray cast, but over what the *writer* will actually
    // emit: `plan_scene3d()`'s order, each entry's own pixel ring (the piece's
    // where it was split, the plan's where it was not), and the depth taken
    // from its parent's plane -- a piece is coplanar with the polygon it was
    // cut from, so that plane is the right one for the intersection.
    {
        ...
        s->zmin =  0; s->zmax = 3; s->zlim_auto = false;

        Bar3DPlot bar;
        bar.u = gu; bar.v = gv; bar.heights = gh;
        bar.u_width = bar.v_width = 0.7;
        bar.opts.shading = 0.0f;
        s->bars3d.push_back(std::move(bar));

        // The cut: a YZ plane at x = 0, straight through the middle of a grid
        // that spans x -3..3, carrying a heatmap so it is a picture and not a
        // wash.
        PlaneSnapshot pl;
        ...
        // Every emitted thing as (pixel ring, box-space plane), which is all
        // the oracle needs. A plane contributes its quad, once, because its
        // layers are coplanar and share a position.
        struct Emitted { std::vector<float> xy; Vec3 p0, n; bool ok = false; };
        std::vector<Emitted> em;
        em.reserve(scene.size());
        for (const ScenePaint& sp : scene) {
            ...
            if (sp.kind == ScenePaint::Kind::Bar && sp.index < bplan.size()) {
                ...
                if (!sp.xy.empty()) e.xy = sp.xy;
                    ...
                    std::vector<Px3> r;
                    pj.project_polygon(quad, r);
                    for (const Px3& q : r) { e.xy.push_back(q.x); e.xy.push_back(q.y); }
                }
            }
        ...
        // `SEXTANT_NEWELL=0` is the negative control rather than a skip: the
        // whole-object interleave the SVG writer used before this step, in the
        // same binary, over the same scene, judged by the same oracle -- which
        // must reject it. Without that, every number above would be a
        // statement about the oracle rather than about the algorithm.
        bool newell_on = true;
        if (const char* env = std::getenv("SEXTANT_NEWELL"))
            newell_on = std::atoi(env) != 0;
        if (newell_on)
