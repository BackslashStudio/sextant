// Camera3D: navigation, the per-frame fit, perspective, and the edit lane.
//
...
// Navigation, checked as arithmetic. Every rule about what a gesture *means*
// lives in coord_transform3d.h precisely so it can be asserted without a
// window, an ImGui context or a mouse -- the same reason pan_limits() and
// zoom_limits() live next to CoordTransform.
void test_camera_navigation() {
    std::printf("\n[3D: navigation]\n");

    using namespace sextant;

    // ---- Orbit. The content follows the cursor, which is the convention
    // pan_limits() already states for 2D: dragging right turns the box as
    // though it had been grabbed and pulled right, so the camera goes the
    // other way.
    ...
    // Elevation is where the camera basis collapses, so it clamps rather than
    // being allowed through -- a drag has no idea where 90 degrees is.
    ...
    // ---- Scroll zooms, one notch at a time, and cannot run away.
    ...
    // ---- Fly. A/D strafe along the camera's own right vector, so they move
    // across the *screen* whatever angle the box is seen from; Q/E are world
    // up, because "up" in a plot means z and a tilted camera must not change
    // what that means.
    ...
    // W/S under a parallel projection: translating along the view direction
    // changes nothing at all, so they drive the zoom instead. A literal dolly
    // here would be a dead key that reads as a bug.
    in.forward = true;
    ...
    // Movement is per *second*, not per frame, or the camera would fly at a
    // speed that depended on the frame rate.
    FlyInput slow = in, fast = in;
    slow.right = fast.right = true;
    slow.dt = 0.1; fast.dt = 0.2;
    ...
    // ---- The clamp is the one thing every path above shares.
    Camera3D bad;
    bad.zoom = 0.0;
...
// The fit is a function of the camera *angle*; the target only translates.
// Getting that wrong is invisible until something moves the target -- which
// is exactly what step 2 added -- and then panning silently zooms out.
void test_camera_fit_and_pan() {
    ...
    // ...and it translates the whole picture by exactly the same amount, so
    // the gesture is a pan rather than a pan-and-shrink.
    ...
    // The camera angle is what the fit does answer to -- otherwise the check
    // above would pass on a fit that was simply constant.
...
// ---------------------------------------------------------------------------
// Perspective (v1.0 step 3)
// ---------------------------------------------------------------------------
// The same split step 1 used: the projection is checked against arithmetic
// written out here, not against a picture. A perspective camera has exactly
// one number that is stored nowhere -- the eye distance, which the fit derives
// from the field of view -- so that is where this starts, at a camera chosen
// so the whole of it can be done by hand.
//
...
void test_perspective_projection() {
    ...
    // The fit is tight at the corner that binds -- the near one, which lands
    // exactly on the frame corner -- and every other corner is pulled in by
    // its own eye depth. Both numbers are written out rather than compared to
    // each other, so a fit that was merely self-consistent would fail.
    ...
    // Foreshortening, as a ratio rather than an inequality: the width of a
    // face is inversely proportional to its depth, exactly.
    ...
    // The orthographic control. Nothing above can be true of it, and w is the
    // constant that makes every clip below a no-op.
    Camera3D ocam = cam;
    ...
    // Zoom and the dolly are different controls and the difference is
    // measurable: zoom magnifies the finished picture, so every offset from
    // the frame centre scales and the amount of perspective is untouched.
    Camera3D zoomed = cam;
    zoomed.zoom = 2.0;
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (!near_px(b.x - kCx, 2.0f * (a.x - kCx), 0.01f) ||
            !near_px(b.y - kCy, 2.0f * (a.y - kCy), 0.01f)) magnified = false;
    }
    ...
    // A dolly moves the target along the view direction. The fit answers to
    // the camera *angle* and never to the target (the property step 2 had to
    // fix for the pan), which is exactly what stops the fit from undoing the
    // approach: the derived distance is unchanged and the picture is not.
    ...
    // ...and that is what W/S is wired to under perspective, where a dolly is
    // visible, rather than to the zoom an orthographic camera needs.
    ...
    // Field of view is the one knob, and it is bounded on both sides.
    Camera3D wide = cam, narrow = cam;
    wide.fov = 110.0; narrow.fov = 15.0;
    ...
    // ...and the projector refuses one too, rather than turning the whole
    // projection into NaN for a Camera3D that was built directly instead of
    // through the clamped public setters.
    ...
    // The box fits its cell under perspective too, at every angle and every
    // fov -- which is the whole claim that there is no distance to tune. Swept
    // over an oblong box, because a cube hides which corner binds.
    ...
    for (const double fov : { 20.0, 45.0, 90.0, 115.0 }) {
        for (int ai = 0; ai < 8; ++ai) {
            for (int ei = -2; ei <= 2; ++ei) {
                ...
                for (int i = 0; i < 8; ++i) {
                    const Vec3 corner{ (i & 1) ? 0.8 : -0.8, (i & 2) ? 0.5 : -0.5,
                                       (i & 4) ? 0.35 : -0.35 };
                ...
                if (std::fabs((hi_x - lo_x) - iw) < 0.5f ||
                    std::fabs((hi_y - lo_y) - ih) < 0.5f) tight = true;
            }
        }
    }
...
// The near plane. A perspective camera can put part of the box behind the eye
// -- fly into it and it will -- and the projection of a point behind the eye
// is not off-screen but *wrong*: it lands on the opposite side of the picture.
// This is the one piece of arithmetic in 3D with no 2D precedent, so it gets
// its own group, with the fault it prevents asserted rather than described.
void test_perspective_near_clipping() {
    ...
    // Flown 1.2 box units forward from a camera one unit out: the eye ends up
    // at x = -0.2, inside a box spanning -0.5..0.5, so the +x half of it is
    // behind the eye and the -x half is in front.
    Camera3D cam;
    cam.azimuth = 0.0; cam.elevation = 0.0;
    cam.projection = Projection::Perspective;
    cam.fov = 90.0;
    ...
    // A straddling segment is cut at the plane rather than dropped, and the
    // surviving end is the one that was in front.
    ...
    // The fault this prevents, asserted rather than described. Both endpoints
    // are on the +y side of the box, which this camera puts to screen right;
    // projecting the behind-the-eye one without clipping negates the divide
    // and throws it to screen *left*, so an unclipped axis line would be drawn
    // straight across the picture. No check on the drawn geometry alone would
    // notice -- it is still a line, still finite, still inside the frame.
    ...
    // Polygons take the same plane, one Sutherland-Hodgman pass.
    std::vector<Px3> poly;
    p.project_polygon({ { 0.5, -0.5, -0.5 }, { 0.5, 0.5, -0.5 },
                        { 0.5, 0.5, 0.5 },   { 0.5, -0.5, 0.5 } }, poly);
    ...
    // The whole box plan through the same camera: finite everywhere, and
    // visibly shorter than the same plan from outside. Ticks are the countable
    // half of that -- the ones on the near half of each axis are behind the
    // eye and go, marks and numbers together.
    RenderSnapshot3D snap;
    snap.xticks_override = std::vector<Tick>{ {0.0,"0"}, {0.25,"a"}, {0.5,"b"},
                                              {0.75,"c"}, {1.0,"1"} };
    ...
    outside.camera.azimuth = 0.0; outside.camera.elevation = 0.0;
    snap.camera = cam;
    ...
    bool finite = true;
    for (const auto* g : { &plan_in.panes, &plan_in.grid, &plan_in.axis_lines,
                           &plan_in.tick_marks })
    ...
    // This camera keeps the x and z labelled edges (both on the -x side of the
    // box, in front of the eye) and loses the y one entirely, since both its
    // ends sit at x = +0.5, behind. An axis with nothing in front of the eye
    // is dropped whole -- line, marks, numbers and title -- because every
    // direction the rest of the plan derives comes from where the edge landed.
    ...
    // The surviving ticks, counted a second way -- straight off the projector,
    // from the two edges this camera keeps -- rather than by comparing one
    // plan against another. A count alone would not do it: the y axis being
    // dropped already makes plan_in smaller than plan_out, so "fewer ticks"
    // passes even with the per-tick test removed entirely.
    std::size_t expect = 0;
...
// The camera rides the same FigureEditBox channel pan/zoom uses, which is what
// makes a dragged view survive refresh() and lets a caller script the same
// thing. Here: that the lane exists, merges, and is seen by the drain.
...
void test_camera_edit_lane() {
    ...
    // FigureEdits::empty() is tested by the drain, not per_axes.empty() -- so
    // a lane whose field is missing from empty() is a control that silently
    // does nothing until an unrelated edit happens to ride along with it.
    // That trap is why this check is here rather than assumed.
    ...
    // Merge semantics, as for the 2D lane: many small deltas accumulate
    // between drains rather than overwriting each other. A drag pushes a
    // camera every frame while a panel field pushes a box style; losing
    ...
    // The camera reaches the picture through the public API, which is the
    // half the panel does not exercise.
