// The inverse layout: a wanted frame back to a figure size, and subplot spans.
//
...
void test_frame_size_round_trip() {
    ...
    const Case cases[] = {
        ...
        { "3x1 column",             3, 1, 3,  2 },
        // Spans: the inverse has to take the gaps inside one back out.
...
void test_frame_size_responds_to_layout() {
    ...
    for (auto& fa : with_titles.axes) { fa.snap2d()->title = "T"; fa.snap2d()->xtitle = "x"; fa.snap2d()->ytitle = "y"; }
...
// A subplot covering several cells of the figure's one grid, plus the
// shape-less add_subplot(index) / add_subplot({first, last}). Mixed grid
// shapes stay forbidden; a span is how a wide subplot is had instead, and a
// Figure refuses anything that would make two subplots share a cell.
void test_subplot_spans() {
    ...
    // ---- The geometry: a span's cell runs from its first cell's left/top to
    // its last cell's right/bottom, gaps included, and its frame lines up with
    // the outer frames of the cells it covers.
    {
        auto fs = make_snapshot(2, 3, 4);     // cells 1, 2, 3 and 4
        fs.axes[3].slot.last = 6;             // 4 becomes the bottom row
        fs.col_gap = 17.0f; fs.row_gap = 11.0f;
        fs.margins = { 30.0f, 20.0f, 15.0f, 25.0f };
        for (auto& fa : fs.axes) { fa.snap2d()->title = "T"; fa.snap2d()->ytitle = "y"; }
        ...
        // A click in the column gap inside the span belongs to the span.
        std::vector<AxesLayout> al;
        for (const auto& c : lay.cells) {
            AxesLayout a; a.slot = c.slot; a.cell = c.cell; al.push_back(a);
        }
        if (c1 && sp) {
