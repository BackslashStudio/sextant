// The 2D line's `loop` (v1.0 step 18): Line3DOptions::loop given to
// Axes::plot() and Plane2D::line(), with the same meaning in all three.
// segment_count()/segment_ends() are the definition; what is checked here is
// that the stroke shader's instances, the SVG element and a plane's sheet
// strokes all read it rather than each deriving the wrap again.
//
...
// One 2D axes over fixed 0..10 limits holding `lp` -- errorbar2d.cpp's
// builder, kept local for the same reason it is local there: a builder used
// by exactly one subject belongs in that subject's file.
...
// A triangle, drawn thick enough that a probe a few pixels wide lands on the
// stroke rather than between two of its rows. Its three points are the whole
// of the path: the fourth side exists only if `loop` draws it.
...
// The first element of `tag` in the SVG, or "" -- enough to tell a <polygon>
// from a <polyline> and to count the points either one carries.
...
void test_line_loop_segments() {
    ...
    // A two-point path is the smallest that draws at all, and its loop is the
    // same segment back again -- Line3DPlot's rule, stated here so the two
    // cannot drift apart.
    LinePlot pair;
    ...
    // The limits. `loop` draws a segment and adds no point, so nothing an
    // auto-scaled axes resolves to can move.
    {
...
void test_line_loop_rendered() {
...

    {
        ...
        DataRenderer data;
        // One DataRenderer for both, and **the same data generation**: the two
        // figures differ in `loop` alone, which no data change reports. If the
        // stroke cache is not keyed on it, the second render hands back the
        // first's point buffer and the seam is missing.
        ...
        // The reference the seam is measured against: an *open* path whose
        // middle vertex is the triangle's first point, arrived at and left
        // along the same two directions. That vertex is an ordinary interior
        // bend, and what a closed path claims is that its seam is one too.
    ...
    // The midpoint of the closing side, (5,8) back to (2,2) -- ink there is
    // drawn by the closing segment and by nothing else in the figure.
    ...
    // The seam is an ordinary interior bend, which is a stronger claim than
    // "something is drawn there" and the one that catches the half-done
    // implementation: pad the buffer the way an open path does, raise the
    // instance count by one, and the closing segment appears while the corner
    // it turns is butt-capped on one side, leaving a notch of background
    // outside it. Measured against the same bend in an open path, so the two
    // pictures are of the same corner and the counts must agree exactly.
    {
    ...
    // The SVG. A closed path is the element that closes it, and it carries the
    // same points as the open one -- a first point written twice would be the
    // other way of drawing this, and is not what ships.
    {
    ...
    // A plane's sheet strokes, which read the same two accessors.
    {
