// 2D error bars (v1.0 step 16): the ErrorBar parameter, offsets and the
// symmetric rule, the chevron cap, and what each output draws from them.
//
...
// One 2D axes over fixed 0..10 limits holding `lp`, so a test can state
// pixel positions through the cell's own transform.
...
struct Seg2 { double x0, y0, x1, y1; };
...
void test_errorbar_api() {
    ...
    const std::vector<double> odd{ -0.5, std::numeric_limits<double>::quiet_NaN(),
    ...
    // Through the public API to the file: the empty ErrorBar is the overload
    // without one, and one with data reaches the writer.
    {
...
void test_errorbar_rendered() {
    ...
    struct Case { const char* name; LinePlot lp; };
...

    {
        ...
        DataRenderer data;
        // One DataRenderer for all four, so each needs a generation of its own
        // or the error-bar cache hands the next case the last one's geometry.
...
void test_errorbar_hint_and_rows() {
    ...
    RenderSnapshot snap;
    snap.xmin = 0.0; snap.xmax = 10.0; snap.ymin = 0.0; snap.ymax = 10.0;
