// Plane2D rendered -- one plane, crossing planes, and every 2D kind on one.
//
...
void test_plane2d_render() {
    ...
    // Distinct values, so every cell has its own colormap entry and a pixel
    // says which cell it came from.
    std::vector<float> data(R * C);
    for (int i = 0; i < R * C; ++i)
        data[static_cast<std::size_t>(i)] = static_cast<float>(i) / (R * C - 1);

    // A bare box: no panes, no grid, no ticks, no titles, so the only thing in
    // the picture other than three axis lines is the plane itself.
    auto scene = [&](PlaneOrientation orient, double offset, const char* origin,
        ...
        HeatmapOptions ho; ho.origin = origin;
    ...
    // Every cell's own centre, projected by the *CPU* projector, must carry
    // that cell's own colormap colour in the *GPU's* output. That is the whole
    // statement at once: the quad is where the projector says, the texture is
    // the right way up, and the MVP the shader was fed is the projection the
    // annotation is drawn with. Counting colours would pass on a plane placed
    ...
    auto check_cells = [&](const FigureSnapshot& fs, const std::string& stem,
        ...
        for (int r = 0; r < R && px; ++r) {
            ...
            for (int c = 0; c < C; ++c) {
                ...
                double coord[3];
                coord[m.u] = u; coord[m.v] = v; coord[m.h] = pl.offset;
                ...
                const unsigned char* e =
                    lut + static_cast<int>(
                        ...
                        * 255.0f) * 4;
                if (p[0] == e[0] && p[1] == e[1] && p[2] == e[2]) ++hits; else ++misses;
            }
        }
    ...
    // ---- SVG: the warp problem, and the split by projection mode (§10).
    //
    // Orthographic. A rectangle projects to a parallelogram, so matrix(...) is
    // exact -- and the writer emits the *plan's* six numbers rather than
    // projecting anything itself, which is what the string search asserts.
    {
        ...
        // The matrix is exact, not approximately so: it maps the unit image
        // square onto the three projected corners it was built from, and the
        // fourth lands where a parallelogram says it must.
        ...
        const int corner_of_uv[4][3] = { { 0, 0, 3 }, { 1, 0, 2 },
                                         { 0, 1, 0 }, { 1, 1, 1 } };
    ...
    // Perspective: an affine transform cannot express a projective warp, so
    // the plane comes out one polygon per cell instead. Panes are off in this
    // scene, so every <polygon> in the file is a cell.
    {
        const FigureSnapshot fs = scene(PlaneOrientation::XY, 0.5, "lower",
    ...
    // Above the cell cap the exact form is not an SVG anybody wants, so it
    // falls back to the affine image and *says so* in the file -- which is
    // where a warning about a file belongs.
    {
    ...
    // ---- Emission order. SVG has no depth buffer, so the file *is* the
    // painter's algorithm, and a plane and a bar3d object have no separating
    // plane between them -- only the admitted centre-distance heuristic. What
    // must hold is that the heuristic is actually applied: the two plans are
    // merged, not concatenated, so a slice clear of a bar grid is painted after
    // it when it is in front and before it when it is behind.
    //
    // Checked against distances derived here rather than against the order
    // itself, which is the lesson the bar3d sort taught: an assertion that
    // compares the output with the key that produced it cannot fail.
    //
    // **What this deliberately does not cover**: the planes here are clear of
    // the bar, above and below it. A plane that lies *among* a grid, or cuts
    // through one, is ordered wrongly by the whole-object merge and is a known
    // flaw rather than an untested case -- see spec_3d.md §10, "Known flaw:
    // whole-object ordering is too coarse for planes". Pinning the wrong
    // behaviour in a check would make it look intended, so the scene is the
    // one the heuristic is right for and the flaw lives in the spec.
    {
        FigureSnapshot fs = scene(PlaneOrientation::XY, 0.05, "lower",
                                  Projection::Orthographic);
        RenderSnapshot3D* s = fs.axes[0].snap3d();
        // A second plane at the other end of z, so one is in front of the bar
        // and one behind whichever way the camera faces.
        ...
        // Where each object is, computed independently of the plan.
        ...
        // Each plane's own place in the file, found by its matrix -- which
        // differs between the two because they sit at different offsets.
        ...
        // **Since step 9 the property has changed, and become the stronger
        // one.** Under the whole-object order every plane was emitted either
        // before *all* the bar faces or after *all* of them, and that is
        // exactly memory/spec_3d.md §10's flaw stated as a position in a file:
        // a plane cutting through a grid painted over the half that should
        // occlude it, because there was nowhere else to put it. Newell's
        // algorithm splits the faces the plane passes through, so a plane the
        // grid straddles now has bar faces on **both** sides of it -- which no
        // whole-object order can produce, at any granularity, ever.
    ...
    // Translucency: a plane below 1 has to be composited, not tinted, and the
    // depth buffer alone cannot decide the order. The cheapest statement that
    // it really is see-through is that a bar behind it survives into the
    // picture -- an opaque plane in front hides it outright.
    {
    ...
    // A dashed line on a plane now dashes, which is a limitation §4 recorded
    // rather than a feature anyone asked for -- so it is asserted rather than
    // quietly un-recorded. The in-plane ribbon expanded before it transformed
    // and so had no pixel arc length to carry a dash pattern along; a plane's
    // strokes now go through the ordinary 2D stroke shader, which has one.
    //
    // Counted against the *solid* rendering of the same line rather than
    // against a threshold: "some pixels are missing" only means something
    // beside the picture that has them all.
    {
    ...
    // ---- A *translucent primitive inside* a plane, which is the one thing
    // that can tell the plane's raster apart from a straight-alpha one.
    //
    // The raster is accumulated over transparent black, so its alpha channel
    // has to sum coverage (GL_ONE) while its colour channels take the ordinary
    // src-alpha blend -- glBlendFuncSeparate, not glBlendFunc. Every other
    // check in this file draws *opaque* content onto a plane, and at
    // alpha_src = 1 the two funcs give the same answer, so none of them can
    // see the difference. Here a half-transparent bar over a known background
    // makes it a number: with the separate func the raster carries coverage
    // 0.5 and the picture is half bar and half background; with the plain one
    // it carries 0.25 and three quarters of the background comes through.
    {
        FigureSnapshot fs = scene(PlaneOrientation::XY, 0.5, "lower",
                                  Projection::Orthographic);
        RenderSnapshot3D* s = fs.axes[0].snap3d();
        ...
        if (px) {
            // The background, read from the picture rather than assumed, so
            // this does not quietly depend on the figure's default colour.
            const unsigned char* bg = px;   // top-left, outside the box
            // Where the bar covers, the red channel is raised and the green
            // one is pulled toward zero by exactly the bar's coverage.
            ...
            // Not zero: the bar's antialiased edge sweeps the whole range
            // between the two values, so a handful of pixels legitimately sit
            // at the wrong one. What cannot happen is the *body* of the bar
            // being there -- under a straight-alpha raster the two counts
            // swap outright, so a factor of a hundred separates the cases.
    ...
    // ---- Two planes, two rasters. Every 2D cache in DataRenderer is keyed on
    // the plane being drawn into as well as the plot index, and without that
    // two planes each carrying a line at index 0 share one entry: the second
    // finds it already current for this data generation and draws the first
    // one's geometry. The picture then has one line twice instead of two
    // lines, which is invisible unless the two are told apart by something.
    // They are told apart here by colour.
    {
        FigureSnapshot fs = scene(PlaneOrientation::XY, 0.25, "lower",
                                  Projection::Orthographic);
        RenderSnapshot3D* s = fs.axes[0].snap3d();
        s->planes[0].sheet.heatmaps.clear();
        // Same point *count* on both, different lengths. A cache keyed only on
        // the plot index would find the first plane's buffer current and reuse
        // it wholesale -- so the two lines have to differ in something the
        // stale check cannot notice on its own, which a differing vertex count
        // would be. Colour is a uniform rather than cached geometry, so
        // telling them apart by colour alone would *not* catch it: the second
        // plane would draw the first one's line in its own colour and every
        // colour the picture is supposed to have would still be there.
        ...
        // The short line must come out short. Both present is not enough --
        // that passes on a second plane drawing the first one's geometry in
        // its own colour, which is exactly the failure a shared cache entry
        // produces.
...
// A plane carrying the vector-shaped kinds, built directly so the tests can
// state exactly what is on it.
sextant::PlaneSnapshot make_kinds_plane(sextant::PlaneOrientation o, double offset) {
    ...
    // A box makes the error bars contribute a *fill* as well as
    // strokes, which is what forces the batch list to alternate; the cap data
    // adds the capped whisker over it, so both error-bar shapes are present.
...
// -------------------------------------------------------------------------
// Step 7b: two translucent planes that cross
// -------------------------------------------------------------------------
// The picture a whole-object order cannot draw at all. Two translucent planes
// that cross have no single order: each is in front of the other on one side
// of their intersection line, so *any* answer naming one of them "the near
// one" is wrong over half the overlap. Since 7a a plane is one flat quad with
// a single depth per pixel, which is exactly the condition that lets the
// composite sort them at the fragment instead.
//
// The oracle is deliberately not the projector's depth, which is the quantity
// the shader itself sorts by. It is the hit point's extent along
// Projector3D::eye_dir() -- the camera basis, one step earlier than any
// projection -- so a sign error in the comparator cannot make the test agree
// with the renderer (spec_3d.md §13).
void test_plane2d_composite() {
    ...
    // Two flat colours, at the two ends of the colormap. vmin/vmax are fixed
    // at 0..1 by default, so a constant field is a constant colour and the
    // expected pixel is arithmetic rather than another sample of the picture.
    ...
    // Stay off the heatmap's own edge, where GL_LINEAR is mixing ink with the
    // raster's transparent margin, and off the crossing line itself, where the
    // two hits are the same point and no order is meaningful.
    ...
    // The box's three labelled edges are drawn in pass 3, over the planes, so
    // a handful of samples are a line rather than a blend. Under a whole-object
    // order one of the two regions is wrong outright -- thousands, not a rim.
    check(bad * 50 < ok,
          "plane composite: both sides of the intersection blend in their own order");

    // The vector path is unchanged and still whole-object, which is a named
    // divergence rather than a silent one (spec_3d.md §10). Stated here so the
    // *size* of the disagreement is on the record: every pixel on one side.
    const std::vector<PlanePlanItem> plan = plan_planes3d(proj, s->planes);
    ...
    // The depth the composite writes is the sample's own, not the fragment's
    // nearest -- so an opaque bar in front of a plane still hides it. Here the
    // classification is by Px3::depth, which is what plane_ray_hit() and
    // box_ray_hit() answer in and the one currency a bar and a plane compare
    // in at all; the crossing above is what carries the independent oracle.
    {
                ...
                for (int x = static_cast<int>(fr.x) + 1;
                    ...
                    for (int i = 0; i < 2 && hit; ++i)
                        hit = plane_ray_hit(proj, s->planes[i].orient, s->planes[i].offset,
                              ...
                              && inked(u[i]) && inked(v[i]);
                    if (!hit) continue;
                    // Both planes strictly behind the bar's near face: nothing
                    // of either may reach the picture.
                    if (d[0] < bd + 0.002f || d[1] < bd + 0.002f) continue;
                    const unsigned char* g = barred.at(x, y);
                    if (g[0] > 250 && g[1] < 5 && g[2] < 5) ++pure; else ++tinted;
                }
            }
...
void test_plane2d_kinds() {
    ...
    // ---- The batch list *is* the 2D painter order. Three passes over three
    // primitive lists would be simpler and would get the layering wrong: on a
    // 2D axes the order alternates fill, stroke, fill, stroke, markers, and
    // since every primitive on a plane is exactly coplanar no depth test can
    // recover it.
    using Kind = PlaneGeometry::Batch::Kind;
    std::vector<Kind> kinds;
    for (const auto& b : g.batches) kinds.push_back(b.kind);
    check(kinds == std::vector<Kind>({ Kind::Tri, Kind::Seg, Kind::Tri,
                                       Kind::Seg, Kind::Marker }),
          "kinds: the batches run fill, stroke, fill, stroke, markers -- the 2D order");

    // Contiguous and complete: every primitive is in exactly one batch, so
    // nothing can be built and silently never drawn.
    std::size_t tri_end = 0, seg_end = 0, mark_end = 0;
    bool contiguous = true;
    for (const auto& b : g.batches) {
        std::size_t& end = (b.kind == Kind::Tri)  ? tri_end
                         : (b.kind == Kind::Seg)  ? seg_end : mark_end;
    ...
    // The counts, against what the shapes have to be. Bars: 3 bodies = 6
    // triangles; error boxes: 3 more = 6. Strokes: 3 bar outlines x 4 sides,
    // 3 line segments, 3 box outlines x 4 sides, and 3 whiskers with 2 caps
    // each. Markers: 2 scatter + 1 scatter_z.
    ...
    // Everything lands *on* the plane -- the offset axis is the one the
    // orientation is normal to, and no primitive may leave it.
    bool on_plane = true;
    ...
    // ---- Everything on a plane is *on* the plane, which is step 7a's
    // reversal of the rule this block used to assert.
    //
    // §4's fork put a stroke's width in the scene and a marker's size on the
    // screen, and the check here was that a near plane's stroke thickened
    // while its marker did not. Under the screen model there is nothing left
    // to hold apart: both are rasterized into the plane's own raster, so both
    // foreshorten and recede with it, and `size` names a pixel of that raster
    // rather than of the window (spec_impl.md step 7a, decisions 2 and 4).
    //
    // The two are still checked against each other rather than in isolation,
    // for the reason they were before: "scaled" means nothing on its own. The
    // marker now has to move *with* the stroke -- and a check that only
    // asserted "it changed" would pass on a marker that scaled by some
    // unrelated amount, so the ratio is compared, not the direction.
    {
        // The scene, kept whole rather than reduced to a plan, so the same
        // camera can be asked directly how much the plane shrinks between the
        // two offsets -- an oracle computed from the projector rather than
        // from either of the two quantities being compared.
        auto scene_at = [&](double fov, double offset) {
            FigureSnapshot fs = make_snapshot3d(1, 1, 1);
            RenderSnapshot3D* s = fs.axes[0].snap3d();
            s->camera.projection = Projection::Perspective;
            s->camera.fov = fov;
            s->xmin = 0; s->xmax = 4; s->xlim_auto = false;
            s->ymin = 0; s->ymax = 4; s->ylim_auto = false;
            s->zmin = -1; s->zmax = 1; s->zlim_auto = false;
        ...
        // How many pixels one box unit of the plane covers, at the marker's
        // own position: the plane's projected scale, straight from the camera.
        ...
        // *By how much*, against the camera rather than against the other
        // quantity. "It got bigger" would pass on a marker that scaled by any
        // unrelated amount -- and the two measurements above are taken at
        // different points on the plane, which under perspective have
        // different scales, so they are not each other's oracle. The plane's
        // own projected scale at the marker's position is.
    ...
    // ---- The ribbon lies *in* the plane, not across it.
    //
    // A stroke billboarded toward the eye keeps its width whatever angle the
    // surface is seen at, and puts half of that width in front of the surface
    // and half behind -- which is z-fighting against whatever else is on the
    // plane, and an outline that comes out one thickness inside its own bar
    // and another outside it. An in-plane ribbon is ink on paper: it
    // foreshortens with the surface and cannot fight it.
    //
    // Checked against the projected length of the segment *itself*, which at
    // azimuth 0 runs across the screen at either elevation -- so the fit's own
    // change of scale cancels and what is left is the foreshortening alone.
    {
        auto at_elev = [&](double elev, float& width, float& along) {
            ...
            sn->xmin = 0; sn->xmax = 4; sn->xlim_auto = false;
            sn->ymin = 0; sn->ymax = 4; sn->ylim_auto = false;
            sn->zmin = -1; sn->zmax = 1; sn->zlim_auto = false;
            PlaneSnapshot p;
            p.orient = PlaneOrientation::XY;
            LinePlot lp;
            // Constant x, so the segment runs along y -- which azimuth 0 puts
            // across the screen, leaving its in-plane perpendicular (x) as the
            // direction that foreshortens.
    ...
    // ---- Rendered. Same statement the heatmap cell check makes, and the one
    // that catches a plane whose geometry is built and then never reaches the
    // screen: a bar's own centre, projected by the CPU, must carry the bar's
    // colour in the GPU's output.
    ...
    // A bare box holding one plane: a flat heatmap backdrop and, **on the same
    // plane**, one opaque bar over it. Coplanar by construction, which is the
    // case the depth func has to get right -- GL_LESS would reject the bar
    // outright and the picture would silently lose everything but the
    // backdrop. That really happened, with a polygon offset rather than a
    // depth func; see DataRenderer::draw_plane_geometry().
    FigureSnapshot fs = make_snapshot3d(1, 1, 1);
    {
        RenderSnapshot3D* s = fs.axes[0].snap3d();
        s->box_style.panes = false;
        ...
        // A thick outline and an error box, both coplanar with the bar and
        // with each other. This is the arrangement that showed z-fighting: a
        // billboarded stroke puts half its width in front of the surface and
        // half behind, so an outline fought its own bar and came out a
        // different thickness inside the fill than outside it.
...

    {
        ...
        int red = 0, probes = 0;
        // Points inside the bar (x 1..3, y 0..3) and clear of its edges.
    ...
    // PNG and SVG place it identically: the writer carries the plan's own
    // pixels, as it does for the box and the bars.
    {
        const FigureLayout lay = compute_figure_layout(fs, W, H);
        const std::vector<PlanePlanItem> plan =
            plan_planes3d(lay.cells[0].box3d->proj, fs.axes[0].snap3d()->planes);
...
void test_plane2d_legend_and_contours() {
    ...
    // ---- The legend. Its keys are the planes' keys, in plane order, through
    // the same collect_legend_entries() a 2D axes uses -- so a label reaches
    // a 3D cell's legend by being on a plane, not by any new mechanism.
    FigureSnapshot fs = make_snapshot3d(1, 1, 1);
    RenderSnapshot3D* s = fs.axes[0].snap3d();
    ...
    // ---- And with the *shape* each marker series calls for, which is a bug
    // fix rather than a step-6 property. Both renderers drew a hardcoded
    // circle, so a Square or a Diamond series was keyed by a circle in the
    // window *and* in the SVG: the two outputs agreed with each other while
    // both disagreed with the picture, which is why nothing caught it.
    check(dec.legend_entries[1].marker == MarkerStyle::Square,
          "legend: a marker key carries the series' own marker shape");

    // The drawn file, not just the entry, since the entry was never the part
    // that was wrong. `make_kinds_plane()`'s only marker series is a Square,
    // so a legend that still hardcoded a disc would put a <circle> in the file
    // and no <rect> of the swatch's size.
    {
    ...
    // Carved out of the cell, exactly as a colorbar is and exactly as a 2D
    // axes' own legend is -- the box is beside the frame, not in the scene.
    ...
    // ---- Contours. Step 5 refused them at ingest; step 6 draws them, as
    // *annotation* -- so their width and their label size are pixel numbers
    // that do not answer to the camera, which is the half of §4's fork the
    // scene geometry beside them deliberately does not get.
    auto contoured = [&](double fov, double offset) {
        ...
        cs->camera.fov = fov;
        cs->xmin = 0; cs->xmax = 8; cs->xlim_auto = false;
        cs->ymin = 0; cs->ymax = 8; cs->ylim_auto = false;
        cs->zmin = -1; cs->zmax = 1; cs->zlim_auto = false;
        PlaneSnapshot p;
        p.orient = PlaneOrientation::XY;
        p.offset = offset;
        HeatmapPlot hp;
    ...
    // The geometry really did move, or the check above would pass on a
    // planner that ignored the camera entirely.
    ...
    // Both outputs draw them, from the one plan.
