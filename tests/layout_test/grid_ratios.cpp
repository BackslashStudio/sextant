// Grid weights (v1.0 step 15.3): columns and rows sharing out the grid by
// weight, the inverse under them, the public setters, the journal that keeps
// a dragged value, and the plot panel's boundary drag.
//
...
// The one figure-level edit that survives refresh(): journaled, latest value
// only, and dropped by an explicit set.
...
void test_grid_boundary_drag() {
    ...
    PanelState st;
    auto ptr = [](float x, float y) { PlotPointer p; p.x = x; p.y = y; p.hovered = true; return p; };
