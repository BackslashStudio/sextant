// surface_tri: ingest and topology, the per-fragment colour, the SVG's
// per-triangle gradient, the keys, the hover/panel, and the Delaunay the two
// `orient` overloads derive their topology with.
//
...
// A unit square as two triangles in the z = `h` plane, with the four corners
// at (0,0) (1,0) (1,1) (0,1). The smallest mesh with more than one face, and
// the one every geometry check below is easiest to reason about.
...
// -------------------------------------------------------------------------
// Step 14.0: the four overloads, the throws, the topology accessors,
// auto-scale, and the has_data gate step 13 shipped broken for its own kind
// -------------------------------------------------------------------------
void test_surface_tri_ingest() {
    ...
    // ---- The vertex rules. Three, not one: a mesh is made of triangles, and
    // two vertices span no area -- surface()'s 2 x 2 argument with the
    // smallest thing that has a face in it rather than a cell.
    ...
    const std::vector<double> nan_z{ 0.0, std::numeric_limits<double>::quiet_NaN(),
                                     1.0, 0.0 };
    ...
    // ---- The topology rules, which are this kind's own.
    ...
    // **Degenerate triangles are kept.** Dropping them would make the caller's
    // face count and ours disagree, and a face count is how a caller checks
    // their own mesh. Asserted here because "skip the zero-area rows" is the
    // obvious implementation and it is the wrong one.
    {
        ...
        // The other half of that bargain: keeping it means the *normal* has
        // to be guarded, or a zero-length cross product would take the face
        // to NaN. It takes the light head-on rather than going black, since
        // black would read as a hole in the sheet.
        SurfaceTriPlot m;
        m.x = x; m.y = y; m.z = z;
        m.tri = flat;
        m.opts.shading = 0.9f;
        Transform3D tf3;
        tf3.xmin = 0; tf3.xmax = 1; tf3.ymin = 0; tf3.ymax = 1;
        tf3.zmin = 0; tf3.zmax = 1;
        SurfaceTriFace face;
    ...
    // ---- The accessors every consumer reads the topology through.
    {
    ...
    // ---- The colour range, on the cloud's and the path's terms.
    {
        SurfaceTriPlot m = unit_quad();
        m.colors = c;
    ...
    // ---- Auto-scale. A mesh's vertices are its extent.
    {
        SurfaceTriPlot m;
        m.x = x; m.y = y; m.z = z;
        m.tri = tri;
        ...
        // A vertex no triangle names is still data the caller gave, and the
        // limits are about the data rather than about what the topology
        // happened to reach. The alternative -- scaling to the vertices in
        // `tri` -- would make the box change when a face is edited out, which
        // is not a statement about the data.
        SurfaceTriPlot lone = m;
        auto lx = x; lx.push_back(5.0);
        auto ly = y; ly.push_back(0.0);
        auto lz = z; lz.push_back(0.0);
        lone.x = lx; lone.y = ly; lone.z = lz;
        const DataBounds3D lb = auto_scale3d({}, {}, {}, {}, {}, { lone }, 0.0);
        check(lb.xmax == 5.0,
              "surface_tri: a vertex named by no triangle still feeds the limits");

        // Interleaving with another kind, which is what says the mesh reaches
        // auto_scale3d() at all rather than being scaled by something else.
        Bar3DPlot bars;
        ...
        // The bar alone sits on one coordinate, which auto_scale3d widens to
        // +-0.5 by its own degenerate-interval rule -- so the number to
        // compare against is that, not the coordinate.
        check(both.xmin == 0.0 && bar_only.xmin > 2.0 && both.xmax == 3.0,
              "surface_tri: and a mesh widens a box the other kinds had to themselves");
    }

    // ---- **prepare()'s has_data gate.** Named in the step's own plan,
    // because step 13 shipped a figure that kept the empty axes' 0..1 for
    // exactly this omission while every unit check above passed: the mesh
    // reaches auto_scale3d() and auto_scale3d() is never called.
    {
...
// -------------------------------------------------------------------------
// Step 14.1: the raster mesh -- per-fragment colour, flat per-face shade
// -------------------------------------------------------------------------
void test_surface_tri_render() {
    ...
    // Looking straight down +x with no elevation: y is screen-horizontal, z is
    // screen-vertical and x is depth, so a mesh at constant x is a flat shape
    // on the screen and every probe below is a pixel in it.
    auto bare = [&]() {
    ...
    auto lut_color = [](Colormap cm, double t, float out[3]) {
        ...
        out[0] = l[0] / 255.0f; out[1] = l[1] / 255.0f; out[2] = l[2] / 255.0f;
    };
    ...
    // ---- **The check that carries the step**: the colour at a point *inside*
    // a triangle, against colormaps::get() of the barycentrically interpolated
    // value.
    //
    // 13.2b's lesson, and it binds harder here. Two candidate rules -- look
    // the colour up per fragment, or look it up at the three corners and let
    // the rasterizer blend -- agree exactly at the vertices and disagree
    // everywhere else, so a check that only probed the corners would pass on
    // both. Where a segment's RGB chord is one straight line off the colormap,
    // a triangle's blend is a whole area of colours the colorbar does not
    // contain, which is why the wrong answer is printed beside the right one.
    {
        ...
        m.opts.vmin = 0.0f; m.opts.vmax = 1.0f;
        ...
        bool read = false;
        if (px) {
            // The centroid: an affine projection takes the triangle's centroid
            // to the centroid of the projected corners, and the barycentric
            // value there is the mean of the three.
            Px3 q[3];
            ...
            // What the rejected rule would have drawn: the mean of the three
            // corners' own colours.
            for (int i = 0; i < 3; ++i) {
                float c3[3];
    ...
    // ---- The shade is flat per face, and two-sided.
    //
    // A "roof": two triangles sharing an edge and tilted oppositely. Each face
    // is one brightness throughout -- that is what "flat" means -- and the two
    // differ, which is what says the shade is per face rather than per mesh.
    {
        FigureSnapshot fs = bare();
        // **Two disjoint faces, not two halves of one quad.** They have to be
        // disjoint *on screen*: two triangles sharing an edge can overlap in
        // projection, and a probe that strayed across the seam would read the
        // other face's shade as a spread within this one -- which is the
        // failure the flatness check is supposed to catch.
        //
        // The camera looks along +x, so the screen is (y, z) and both faces
        // project to proper triangles: the first stands at constant x, the
        // second is tilted, so the two have different normals.
        SurfaceTriPlot m;
        ...
        // The second face is wound the other way round, so the two disagree
        // about which side is "out". With max(0, n.l) one of them would go to
        // black; with |n.l| both are lit -- the rule a mesh needs more than a
        // grid does, since a mesh read out of a file often has no consistent
        // winding at all.
        m.tri = std::vector<std::uint32_t>{ 0, 1, 2, 5, 4, 3 };
        // Not white: a lit face against a white background is a few codes
        // away from it, and a probe that strayed onto the background would
        // read as a pass.
        ...
        bool read = false;
        if (px) {
            // Two probes well inside each face, at 1/4 and 3/4 of the way
            // between its centroid and each of its corners.
    ...
    // ---- The wireframe: all three edges of every face, drawn as geometry.
    {
        auto wire_pixels = [&](bool on) {
            FigureSnapshot fs = bare();
            SurfaceTriPlot m = unit_quad();
            // Pushed into the box's own span, so the mesh is comfortably
            // inside the frame at this camera.
            ...
            int w = 0, h = 0, comp = 0;
            unsigned char* px = stbi_load(on ? "surface_tri_wire_on.png"
...
// -------------------------------------------------------------------------
// Step 14.2: the painter, and the per-triangle gradient in the SVG
// -------------------------------------------------------------------------
void test_surface_tri_svg() {
    ...
    // A mesh that crosses a flat sheet, so faces land both in front of it and
    // behind it -- which is what makes the order question askable in both
    // directions rather than answerable by a depth sort alone. line3d's probe
    // scene with a mesh where the path was.
    auto build = [&](float sheet_alpha) {
        ...
        // A fan of triangles straddling x = 0.5: every face has one vertex on
        // each side, so every one of them is cut.
        SurfaceTriPlot m;
        ...
        for (std::uint32_t i = 0; i + 2 < 8; ++i) { tri.push_back(i); tri.push_back(i + 1);
                                                    tri.push_back(i + 2); }
        m.x = mx; m.y = my; m.z = mz; m.colors = mc;
        m.opts.shading = 0.0f;
        m.tri = tri;
    ...
    for (const float alpha : { 0.45f, 1.0f }) {
        ...
        // ---- The order oracle. For every (mesh face, sheet cell) pair that
        // overlap on screen, the one whose geometry is nearer *at the overlap*
        // must be emitted later. Probed at the mesh face's own centroid, with
        // the two depths taken from where that pixel's ray meets each plane --
        // which needs no correspondence between a screen parameter and a
        // box-space one and so stays exact under perspective.
        ...
        // The depth at which this pixel's ray meets the polygon's own plane.
        ...
        for (std::size_t k = 0; k < scene.size(); ++k) {
            ...
            for (std::size_t i = 0; i + 1 < mr.size(); i += 2) { cx += mr[i]; cy += mr[i + 1]; }
            cx /= static_cast<float>(mr.size() / 2);
            cy /= static_cast<float>(mr.size() / 2);
            // **Several probes per face, not one at the centroid.** A
            // triangle has a depth *range*, so where it stands relative to
            // the sheet can differ from one end of it to the other -- which
            // is precisely what a whole-object order gets wrong and a
            // centroid probe would not notice. Under SEXTANT_NEWELL=0 the
            // centroid alone reports this scene as perfectly ordered.
            std::vector<std::pair<float, float>> probes{ { cx, cy } };
            for (std::size_t i = 0; i + 1 < mr.size(); i += 2)
                probes.push_back({ cx + (mr[i] - cx) * 0.7f,
                                   cy + (mr[i + 1] - cy) * 0.7f });
    ...
    // ---- The gradient. A split piece must re-derive its axis from its *own*
    // ring: the field is defined over the whole of the triangle's plane, so a
    // piece needs no sub-range recovery of the kind a cut segment needs -- but
    // it does need this, or every cut triangle carries its parent's full ramp
    // compressed into a fragment of itself, which looks entirely plausible.
    {
        ...
        while ((p = svg.find("<linearGradient id=\"meshGrad", p)) != std::string::npos) {
            ...
            int stops = 0;
            std::size_t q = 0;
            while ((q = block.find("<stop ", q)) != std::string::npos) { ++stops; q += 6; }
            if (stops < 5) ++stop_short;

            auto attr = [&](const char* name) {
                const std::size_t a = block.find(name);
                return a == std::string::npos ? 0.0f
            ...
            // **The discriminating check.** The axis runs from the polygon's
            // lowest projection onto the gradient direction to its highest, so
            // its *length* is that polygon's own extent along the direction.
            // An implementation that measured the parent triangle instead --
            // which is the plausible mistake, since the coefficients are the
            // parent's -- would give a longer axis on every piece the painter
            // cut, and the ramp inside it would be a compressed copy of the
            // parent's full one.
    ...
    // ---- The degenerate cases fall out to a flat fill rather than to a
    // gradient with no direction: a face seen edge-on has no screen-space
    // gradient at all, and three equal values have no ramp.
    {
        FigureSnapshot fs = make_snapshot3d(1, 1, 1);
        RenderSnapshot3D* s = fs.axes[0].snap3d();
        s->camera.azimuth = 0.0; s->camera.elevation = 0.0;
        s->xmin = 0; s->xmax = 1; s->xlim_auto = false;
        s->ymin = 0; s->ymax = 1; s->ylim_auto = false;
        s->zmin = 0; s->zmax = 1; s->zlim_auto = false;
        SurfaceTriPlot m;
        ...
        // Edge-on: the whole triangle projects to a line, so it has no
        // screen-space gradient either.
        SurfaceTriPlot e;
...
void test_surface_tri_legend_and_colorbar() {
    ...
    // ---- The key. **A colormapped mesh is not keyed**, which is a surface's
    // rule and deliberately not a path's -- and the two now sit a few lines
    // apart in the same collector, which is exactly the kind of thing that
    // gets copied by mistake. So it is asserted against a path in one scene.
    {
...
// -------------------------------------------------------------------------
// Step 14.4: hover at the vertices, the Data-panel tab, and the edit lane
// -------------------------------------------------------------------------
void test_surface_tri_hints_and_panel() {
    ...
    for (int mode = 0; mode < 2; ++mode) {
        ...
        // ---- **A mesh is hovered at its vertices**, on §7b's terms with
        // three for four: a face has three samples and no identity of its own,
        // so what the tooltip reports is the nearest of them -- which is also
        // the index a caller's hint_labels entry is written against.
        int named = 0;
        ...
        // ---- Unlike a path, the *middle* of a face answers, because a mesh
        // is a surface the ray hits rather than a set of samples with ink
        // between them. What it names is the nearest vertex -- so a point
        // three quarters of the way toward one corner names that corner.
        {
        ...
        // ---- A colormapped mesh reports the fourth dimension too, and a
        // hint_label is appended below it.
        {
            RenderSnapshot3D cs;
    ...
    // ---- The mesh competes with the other kinds on one depth list, which is
    // what makes the tooltip agree with the picture rather than with the
    // vector it happens to live in.
    {
        const Projector3D proj(tf, cam, frame, 0.1f);
        RenderSnapshot3D snap;
        snap.surface_tri.push_back(mesh);
        ...
        // The bar's top is well above the mesh, so at the bar's own top centre
        // the near thing is the bar.
    ...
    // ---- The Data panel's table, its topology, and its tab.
    {
        RenderSnapshot3D snap;
        SurfaceTriPlot cm = mesh;
        cm.colors = std::vector<double>{ 11.0, 22.0, 33.0, 44.0 };
    ...
    // ---- The edit lanes: appearance, and a data op.
    {
        ...
        // The address has to be the *kind*: a path and a mesh at index 0 of
        // one axes are two different objects.
        RenderSnapshot3D both;
        both.surface_tri.push_back(mesh);
        Line3DPlot path;
        path.x = mesh.x; path.y = mesh.y; path.z = mesh.z;
...
// -------------------------------------------------------------------------
// Step 14.5: Delaunay, checked against properties rather than against itself
// -------------------------------------------------------------------------
void test_delaunay() {
    ...
    // The three oracles, each independent of our own output:
    //
    //   - the **empty-circumcircle property**, over every (triangle, vertex)
    //     pair. This is the definition of a Delaunay triangulation, so a check
    //     of it is a check against the definition rather than against us.
    //   - **Euler's formula**, which for a triangulation of n points with h on
    //     the convex hull gives exactly 2n - 2 - h triangles. The hull is
    //     computed here by a monotone chain that shares no code with the
    //     triangulator.
    //   - **the union of the triangles is the hull**: their total area equals
    //     the hull's.
    auto hull = [](const std::vector<double>& u, const std::vector<double>& v) {
        ...
        // Distinct points only: a duplicate is not a second hull vertex.
        ...
        // **Collinear points on the boundary are kept** (`< 0`, not `<= 0`),
        // because Euler's 2n - 2 - h counts every point *on* the hull and not
        // the corners of it. A 5x5 grid has four corners and sixteen boundary
        // points, and the two answers differ by twelve triangles.
    ...
    auto check_mesh = [&](const char* what,
        ...
        // 1. Empty circumcircle, over every pair. A strict test with a
        //    tolerance, because four cocircular points -- a regular grid's
        //    cell, the most ordinary input this will ever see -- put a vertex
        //    *on* a circle rather than inside it.
        int violations = 0;
    ...
    // A regular grid -- the most ordinary input, and the one whose cocircular
    // quads a non-strict circumcircle test turns into an oscillation.
    {
        std::vector<double> u, v;
        for (int i = 0; i < 5; ++i)
            for (int j = 0; j < 5; ++j) { u.push_back(i); v.push_back(j); }
        check_mesh("regular grid", u, v);
    }
    // Scattered points, which is what this is actually for. A fixed sequence
    // rather than a random one, so a failure is reproducible.
    {
        std::vector<double> u, v;
        unsigned s = 12345u;
        auto next = [&] { s = s * 1103515245u + 12345u; return ((s >> 16) & 0x7fff) / 32767.0; };
        for (int i = 0; i < 40; ++i) { u.push_back(next()); v.push_back(next()); }
        check_mesh("scattered", u, v);
    }

    // ---- Duplicates keep their slots. Renumbering would have been cheaper
    // here and wrong everywhere else: `colors` and `hint_labels` are
    // index-aligned with the vertices the caller passed, so a triangulation
    // that renumbered would silently re-colour the mesh.
    {
    ...
    // ---- Collinear input has no triangulation and says so, rather than
    // handing back an empty mesh for the caller to render as a blank box.
    {
    ...
    // ---- Through the public API: the two `orient` overloads, what they
    // throw, and the boundary a concave domain runs into.
    {
        auto threw = [](auto&& fn) {
            try { fn(); return false; }
        ...
        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j) {
                ...
                z.push_back(0.1 * i * j);
                // Distinct per vertex, so no derived triangle can carry three
                // equal values and fall back to a flat fill -- which would
                // make the gradient count below a count of something else.
                c.push_back(i * 4 + j);
            }
        ...
        // **What is stored is indices**: the triangulation runs once, at
        // ingest, so downstream there is exactly one kind of mesh and the
        // cost is paid once rather than per frame. Counted through the file,
        // since a colormapped face is filled with a gradient of its own and
        // nothing else in the picture emits one -- 2n - 2 - h is 18 for a
        // 4x4 grid, which is the same number Euler gave above.
        {
        ...
        // The orientation says how the mesh is *made*, and nothing about the
        // drawing consults it afterwards -- which is why it is a parameter
        // rather than matplotlib's hardcoded x-y, and why Plotly had to bolt
        // `delaunayaxis` on later. Asserted against the triangulator itself:
        // the same vertices projected onto two different planes are two
        // different point sets and so two different meshes.
        {
        ...
        // Collinear in the plane given: a throw, not a blank box.
        ...
        // hull. Recorded as a boundary rather than a bug: matplotlib answers
        // it with Triangulation.mask, and masking is deferred. Asserted so the
        // behaviour is the documented one rather than an accident -- an
        // L-shaped point set comes back with the notch bridged, which the hull
        // area is the measure of.
        {
            // A U: a bottom and two arms, and nothing across the top. The
            // domain a caller means is the outline; the convex hull is the
            // whole 4 x 4 square, so the opening is bridged.
            std::vector<double> cx, cy, cz;
            for (int i = 0; i <= 4; ++i) { cx.push_back(i); cy.push_back(0); cz.push_back(0.0); }
            for (int j = 1; j <= 4; ++j) { cx.push_back(0); cy.push_back(j); cz.push_back(0.0); }
            for (int j = 1; j <= 4; ++j) { cx.push_back(4); cy.push_back(j); cz.push_back(0.0); }
